import type { Migration } from '../types'

export const migration0008SystemFileManagement: Migration = {
  id: '0008_system_file_management',
  name: 'add system file management',
  statements: [
    `
      CREATE TABLE IF NOT EXISTS config_file_next (
        id SERIAL PRIMARY KEY,
        config_type VARCHAR(30) NOT NULL CHECK (config_type IN ('site', 'system', 'file')),
        config_key VARCHAR(255) NOT NULL,
        config_value TEXT NOT NULL,
        created_at TIMESTAMPTZ NOT NULL,
        updated_at TIMESTAMPTZ NOT NULL,
        UNIQUE (config_type, config_key)
      )
    
    `,
    `
      INSERT INTO config_file_next (
        id,
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      SELECT
        id,
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      FROM sys_config
      ON CONFLICT DO NOTHING
    `,
    `
      SELECT setval(
        pg_get_serial_sequence('config_file_next', 'id'),
        COALESCE((SELECT MAX(id) FROM config_file_next), 1),
        (SELECT MAX(id) FROM config_file_next) IS NOT NULL
      )
    
    `,
    `DROP TABLE sys_config
    `,
    `ALTER TABLE config_file_next RENAME TO sys_config
    `,
    `
      CREATE TABLE IF NOT EXISTS sys_file (
        id SERIAL PRIMARY KEY,
        upload_type VARCHAR(30) NOT NULL CHECK (
          upload_type IN ('avatar', 'notification', 'page')
        ),
        storage_mode VARCHAR(30) NOT NULL CHECK (storage_mode IN ('local', 's3')),
        storage_key VARCHAR(500) NOT NULL UNIQUE,
        original_name VARCHAR(255) NOT NULL,
        mime_type VARCHAR(255) NOT NULL,
        file_size INT NOT NULL,
        user_id INT,
        created_at TIMESTAMPTZ NOT NULL,
        updated_at TIMESTAMPTZ NOT NULL
      )
    
    `,
    `
      CREATE INDEX IF NOT EXISTS idx_sys_file_upload_type_created_at
      ON sys_file (upload_type, created_at, id)
    
    `,
    `
      CREATE INDEX IF NOT EXISTS idx_sys_file_user_id_created_at
      ON sys_file (user_id, created_at, id)
    
    `,
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_storage_driver',
        'local',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_local_root',
        './uploads',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_s3_endpoint',
        '',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_s3_region',
        'auto',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_s3_bucket',
        '',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_s3_access_key_id',
        '',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_s3_secret_access_key',
        '',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_config (
        config_type,
        config_key,
        config_value,
        created_at,
        updated_at
      )
      VALUES ('file', ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'file_s3_signed_url_ttl_seconds',
        '300',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    `
      INSERT INTO sys_role_menu (
        role_id,
        menu_name,
        created_at,
        updated_at
      )
      VALUES (1, 'admin.system.file', '2026-01-01T00:00:00.000Z', '2026-01-01T00:00:00.000Z')
      ON CONFLICT DO NOTHING
    `,
    {
      sql: `
      INSERT INTO sys_permission (
        code,
        name,
        group_name,
        method_pattern,
        path_pattern,
        action_key,
        sort_order,
        created_at,
        updated_at
      )
      VALUES (?, ?, '文件管理', ?, ?, ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'admin.system.file.view',
        '查看文件列表',
        'GET',
        '/admin/system/file',
        '*',
        800,
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_permission (
        code,
        name,
        group_name,
        method_pattern,
        path_pattern,
        action_key,
        sort_order,
        created_at,
        updated_at
      )
      VALUES (?, ?, '文件管理', ?, ?, ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'admin.system.file.upload',
        '上传文件',
        'POST',
        '/admin/system/file',
        'upload',
        801,
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_permission (
        code,
        name,
        group_name,
        method_pattern,
        path_pattern,
        action_key,
        sort_order,
        created_at,
        updated_at
      )
      VALUES (?, ?, '文件管理', ?, ?, ?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        'admin.system.file.delete',
        '删除文件',
        'POST',
        '/admin/system/file',
        'delete',
        802,
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_role_permission (
        role_id,
        permission_code,
        created_at,
        updated_at
      )
      VALUES (?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        1,
        'admin.system.file.view',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_role_permission (
        role_id,
        permission_code,
        created_at,
        updated_at
      )
      VALUES (?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        1,
        'admin.system.file.upload',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    },
    {
      sql: `
      INSERT INTO sys_role_permission (
        role_id,
        permission_code,
        created_at,
        updated_at
      )
      VALUES (?, ?, ?, ?)
      ON CONFLICT DO NOTHING
      `,
      params: [
        1,
        'admin.system.file.delete',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z'
      ],
    }
  ],
}
