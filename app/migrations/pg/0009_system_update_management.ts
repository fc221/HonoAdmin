...
export const migration0009SystemUpdateManagement: Migration = {
  ...
  statements: [
    ...
    `,
    `
      INSERT INTO config_update_next (
        id,
        config_type,
        config_key,
        config_value,
    ...
    `
      SELECT setval(
        pg_get_serial_sequence('config_update_next', 'id'),
        COALESCE((SELECT MAX(id) FROM config_update_next), 1),
        (SELECT MAX(id) FROM config_update_next) IS NOT NULL
      )
    ...
    `,
    `
      INSERT INTO sys_file_upload_type_next (
        id,
        upload_type,
        storage_mode,
        storage_key,
    ...
    `
      SELECT setval(
        pg_get_serial_sequence('sys_file_upload_type_next', 'id'),
        COALESCE((SELECT MAX(id) FROM sys_file_upload_type_next), 1),
        (SELECT MAX(id) FROM sys_file_upload_type_next) IS NOT NULL
      )
