...
export function hasAdminMenuHref(items: MenuItem[]): boolean {
...
export default function HeaderActions({ menus, user }: HeaderActionsProps) {
  ...
  return (
    <>
      ...
      >
        <i class="icon-[ri--loop-right-line] text-sm"></i>
      </button>
      {canSwitchRole && user
        ? (
            <RoleSwitchDropdown
      ...
      {hasAdminMenuHref(menus)
        ? (
            <a
              ...
              title=""
            >
              <i class="icon-[ri--database-2-line] text-sm"></i>
            </a>
          )
        : null}
      <SettingsDrawer />
...
function UserDropdown({ user }: { user: UserHeaderProfile | null }) {
  return (
  ...
  return (
    <div class="relative" data-header-role-menu="true">
      <button
        ...
        type="button"
      >
        <i class="icon-[ri--shield-user-line] text-sm" aria-hidden="true" />
      </button>
      <ul
