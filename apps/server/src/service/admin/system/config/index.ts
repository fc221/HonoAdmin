...
export interface UpdateConfigValuesInput {
...
export async function updateConfigValues(
  ctx: ServiceContext,
  input: UpdateConfigValuesInput,
): Promise<number> {
  const configs = await listConfigs(ctx)
  const configsByKey = new Map(
    configs
      .filter((config) => config.configType === input.configType)
      .map((config) => [config.configKey, config]),
  )
  let updateCount = 0

  for (const [configKey, configValue] of Object.entries(input.values)) {
    const config = configsByKey.get(configKey)
    if (!config) {
      continue
    }

    await updateConfig(ctx, config.id, { configValue })
    updateCount += 1
  }
