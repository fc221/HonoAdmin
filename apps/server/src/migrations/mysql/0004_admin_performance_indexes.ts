...
export const migration0004AdminPerformanceIndexes: Migration = {
  ...
  statements: [
    `
      CREATE INDEX idx_user_root_status
      ON sys_user (is_root, status)
    
    `,
    `
      CREATE INDEX idx_web_notification_sort
      ON web_notification (is_top, created_at, id)
    
    `,
    `
      CREATE INDEX idx_web_feedback_created_at_id
      ON web_feedback (created_at, id)
    
    `,
    `
      CREATE INDEX idx_operate_log_created_at_id
      ON sys_operate_log (created_at, id)
    
    `,
    `
      CREATE INDEX idx_operate_log_type_created_at_id
