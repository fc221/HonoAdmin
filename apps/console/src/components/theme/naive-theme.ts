...
export function createNaiveThemeOverridesFromDraft(draft: NaiveThemeDraft): GlobalThemeOverrides {
  ...
  return {
    ...
    Menu: {
      // 层级风格(参考组件库):
      //  - hover:中性灰底 + 常规文字(灰用字面值,与全局 hoverColor 解耦,不会变蓝);
      //  - 选中:浅色底(hoverColor)+ 主色文字/图标/箭头,而非实心主色块;
      //  - 含选中子项的父级(child-active):主色文字/图标/箭头。
      arrowColorActive: common.primaryColor,
      arrowColorActiveHover: common.primaryColor,
      ...
      color: 'transparent',
      itemColorActive: common.hoverColor,
      itemColorActiveCollapsed: common.hoverColor,
      itemColorActiveHover: common.hoverColor,
      itemColorHover: 'rgba(128, 128, 128, 0.12)',
