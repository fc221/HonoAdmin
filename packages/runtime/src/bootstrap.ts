  ...
  const requirements: BootstrapConfigRequirement[] = [
    {
      ...
      key: 'DATABASE_URL',
      label: '数据库地址',
      value: values.DATABASE_URL ?? bindings.DATABASE_URL ?? './hono-admin.sqlite',
    },
    {
    ...
    },
    {
      ...
      key: 'CACHE_NAMESPACE',
      label: '缓存命名空间',
      value: values.CACHE_NAMESPACE ?? bindings.CACHE_NAMESPACE ?? 'hono-admin',
    },
    {
...
export async function saveBunRuntimeConfig(
  ...
  const nextValues: Record<BootstrapConfigKey, string | undefined> = {
    ...
    CACHE_NAMESPACE: input.cacheNamespace.trim() || 'hono-admin',
    DATABASE_URL: input.databaseUrl.trim() || './hono-admin.sqlite',
    DB: undefined,
