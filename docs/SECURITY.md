...
- `JWT_SECRET` 必须使用强随机字符串。Cloudflare Workers 使用 `wrangler secret put JWT_SECRET` 配置，本地 Bun 开发由安装页写入环境配置文件。
...
- 生产环境必须使用 HTTPS。
- 不要把 `JWT_SECRET`、对象存储 access key、secret key、数据库 id 等敏感配置提交到公开仓库。
